...
import { Page, AnyBlock, ListCommandBuilder } from "../types";
...
export class History {
  ...
  holdedBuilder?: ListCommandBuilder<any, any>;
  ...
  hold() {
    this.holdedBuilder = new ListCommandBuilder({});
    this.isHold = true;
  }

  release() {
    if (this.holdedBuilder) {
      const command = this.holdedBuilder.build();
      this.isHold = false;
      this.execute(command);
    }
    this.holdedBuilder = undefined;
  }
  /**
  ...
  execute(command: Command<any>, executed?: boolean) {
    command.history = this;
    if (this.isHold) {
      this.holdedBuilder?.addCommand(command);
      return;
    }
