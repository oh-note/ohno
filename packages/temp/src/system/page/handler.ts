...
import { getDefaultRange, tryGetDefaultRange } from "../functional";
import {
  BlockActiveEvent,
  BlockDeActiveEvent,
...
import { isLeftButtonDown } from "../status";
...
export class PageHandler {
  ...
  _findBlocksFromRange(range: Range): { block: AnyBlock; endBlock: AnyBlock } {
    let startTgt = range.startContainer;
    if (range.startContainer === this.page.blockRoot) {
      startTgt = startTgt.childNodes[range.startOffset];
    }

    const block = this.page.findBlock(startTgt);
    if (!block) {
      throw new NoRangeError();
    }
    if (range.collapsed) {
      return { block, endBlock: block };
    }

    let endTgt = range.endContainer;
    if (range.endContainer === this.page.blockRoot) {
      endTgt = range.endContainer.childNodes[range.endOffset - 1];
    }

    const endBlock = this.page.findBlock(endTgt);
    if (!endBlock) {
      throw new NoRangeError();
    }
  ...
  getContextFromRange(
    ...
    const { block, endBlock } = this._findBlocksFromRange(range);

    const context: RangedBlockEventContext = {
      page: this.page,
      block,
      range,
    };
    context.endBlock = range.collapsed ? undefined : endBlock;

    if (endBlock && endBlock !== context.block) {
      context.isMultiBlock = true;
      const blocks = [context.block];
      let cur = context.block;
    ...
    return context as RangedBlockEventContext;
  }
  ...
  handleMouseMove(e: MouseEvent): void | boolean {
    const context = isLeftButtonDown(e)
  ...
  handleMouseUp(e: MouseEvent): void | boolean {
    ...
    let retrive;
    if (range && !range.collapsed) {
      context = this.getContextFromRange(range);
    } else if ((retrive = this.retriveBlockFromMouse(e))) {
      context = {
        block: retrive.block,
        page: this.page,
        range,
    ...
    this.dispatchPageEvent(
      new BlockSelectChangeEvent({ ...context, range, from: "mouseUp" })
    );
  }
