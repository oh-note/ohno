...
export abstract class Block<T extends BlockData = BlockData> implements IBlock {
  ...
  mergeable: boolean = true; // 表格、图片、公式复杂组件等视为独立的 unmergeable，multiblock 下只删除内容，不删除 editable，paste 时在其后创建，而不是将其拆开
  order: Order = "";
  ...
  abstract readonly editables: HTMLElement[];
  ...
  public get commandSet(): CommandSet {
    return this.page.getBlockCommandSet(this.type);
  }
  ...
  getGlobalLocation(bias: number): RefLocation | null {
    return this.selection.biasToLocation(this.inner, bias);
  }
  ...
  getEditablesFromRange(range: Range): HTMLElement[] {
    const startEditable = this.findEditable(range.startContainer);
    const endEditable = this.findEditable(range.endContainer);
    if (!startEditable || !endEditable) {
      throw Error("range not valid");
    }
    let cur = startEditable;
    const res = [];
    while (cur != endEditable) {
      cur = this.getNextEditable(cur)!;
      res.push(cur);
    }
    return res;
  }
  ...
  getEditables(end?: number | undefined): HTMLElement[];
  getEditables(
  ...
  getEditables(
    ...
    const startEditable = !start
      ? this.getFirstEditable()
      : this.getEditable(start);
    const endEditable = !end ? this.getLastEditable() : this.getEditable(end);
    let cur = startEditable;
    const res = [startEditable];
    while (endEditable != cur) {
      cur = this.getNextEditable(cur)!;
      res.push(cur);
    }
    return res;
